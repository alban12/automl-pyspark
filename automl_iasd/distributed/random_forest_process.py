...
def generate_best_feature_subset(full_train_set_dataframe, label_column_name, task, budget):
	...
	train_set_dataframe_for_selection = li_tr.transform(train_set_dataframe) 
	validation_set_dataframe_for_selection = li_va.transform(validation_set_dataframe)
...
space = {
  ...
  'numTrees': hp.uniform('numTrees', 15, 25),
  'maxDepth': hp.uniform('maxDepth', 4, 8)
}
...
best_stages.append(best_logistic_regression)
best_pipeline = Pipeline(stages=stages)
best_model = pipeline.fit(full_train_set_dataframe)

# Evaluate the model 
logging.info("AutoML : Evaluating the performance of the best model on the test set ... ")
predictions = model.transform(test_set_dataframe)
evaluator = BinaryClassificationEvaluator(metricName='areaUnderROC')
evaluator.setLabelCol(f"{label_column_name}")
aucroc = evaluator.evaluate(predictions)
...
s3 = boto3.client('s3')
json_object = {"Algorithm" : algorithm_name,
...
best_model.save(f"{model_path}/{algorithm_name}_{aucroc_on_test}/model")