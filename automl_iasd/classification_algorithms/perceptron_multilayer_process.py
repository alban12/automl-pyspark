...
algorithm_name = "MultilayerPerceptronClassifier"
...
def generate_best_feature_subset(full_train_set_dataframe, label_column_name, task, budget):
	...
	# The basis layers that are set in the nrpa function are : [len(selected_features), len(selected_features), 2]
...
space = {
  ...
  'stepSize': hp.uniform('stepSize', 0.03, 0.04*float(budget)),
  'layers': hp.choice('layers', [ [len(feature_selection_indices), len(feature_selection_indices), 2], [len(feature_selection_indices), len(feature_selection_indices)*2, 2]])
}
...
if best_params["layers"] == 0:
	layers = [len(feature_selection_indices), len(feature_selection_indices), 2]
else:
	layers = [len(feature_selection_indices), len(feature_selection_indices)*2, 2]
...
best_multilayer_perceptron = MultilayerPerceptronClassifier(solver=solver, layers=layers, stepSize=stepSize)
...
s3.put_object(
     ...
     Key=f'{automl_instance_model_path}/MultilayerPerceptronClassifier_{aucroc_on_test}/metrics'
)
# We finally end up by saving the found model 
best_model.save(f"s3a://{bucket_name}/{automl_instance_model_path}/MultilayerPerceptronClassifier_{aucroc_on_test}/model")
