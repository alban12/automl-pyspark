...
def generate_best_feature_subset(full_train_set_dataframe, label_column_name, task, budget):
	...
	bestScore, feature_selection = nrpa_feature_selector(level, iterations, train_set_dataframe_for_selection, validation_set_dataframe_for_selection, feature_space_size, algorithm, selection_initial_uniform_policy, task)
...
s3.put_object(
     ...
     Key=f'{automl_instance_model_path}/SVM_{aucroc_on_test}/metrics'
)
# We finally end up by saving the found model 
best_model.save(f"s3a://{bucket_name}/{automl_instance_model_path}/SVM_{aucroc_on_test}/model")
