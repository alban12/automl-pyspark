...
def generate_best_feature_subset(full_train_set_dataframe, label_column_name, task, budget):
	...
	algorithm = LogisticRegression(maxIter=30)
...
best_logistic_regression = LogisticRegression(regParam=best_params["regParam"], elasticNetParam=best_params["elasticNetParam"])
...
s3 = boto3.client('s3')
json_object = {"Algorithm" : "LogisticRegression",
...
s3.put_object(
     ...
     Key=f'{automl_instance_model_path}/LogisticRegression_{aucroc_on_test}/metrics'
)
# We finally end up by saving the found model 
best_model.save(f"s3a://{bucket_name}/{automl_instance_model_path}/LogisticRegression_{aucroc_on_test}/model")
