...
algorithm_name = "GradientBoostedTree"
...
def generate_best_feature_subset(full_train_set_dataframe, label_column_name, task, budget):
	...
	# Tree algorithms specific preprocessing
	labelIndexer = StringIndexer(inputCol=f"{label_column_name}", outputCol="indexedLabel").fit(train_set_dataframe_for_selection)
	train_set_dataframe_for_selection = labelIndexer.transform(train_set_dataframe_for_selection)
	...
	algorithm = GBTRegressor(maxIter=10)
	...
	classification_algorithms_stages[algorithm_name] = pipeline_stages.copy() # twisted way to do stuff ... to clean 
	...
	featureIndexer = VectorIndexer(inputCol="selectedFeatures", outputCol="indexedFeatures", maxCategories=maximum_number_of_categories)
	...
	return stages, feature_selection, feature_selection_indices, bestScore, train_set_dataframe_for_selection, columns_to_featurized, categorical_columns_to_encode


auto_fe_stages, feature_selection, feature_selection_indices, bestScore, full_train_set_dataframe_with_selection, columns_to_featurized, categorical_columns_to_encode = generate_best_feature_subset(full_train_set_dataframe, label_column_name, task, budget)
...
maximum_number_of_categories = get_max_count_distinct(train_set_dataframe_for_hpo, categorical_columns_to_encode)
...
def train_gradient_boosted_tree(minWeightFractionPerNode, minInstancesPerNode, maxDepth):
	...
	algorithm = GBTRegressor(minWeightFractionPerNode=minWeightFractionPerNode, minInstancesPerNode=minInstancesPerNode, maxDepth=maxDepth)
...
space = {
  ...
  'minInstancesPerNode': scope.int(hp.uniform('minInstancesPerNode', 1, 3)),
...
best_gradient_boosted_tree = GBTRegressor(minWeightFractionPerNode=best_params["minWeightFractionPerNode"], minInstancesPerNode=minInstancesPerNode, maxDepth=maxDepth)
...
s3.put_object(
     ...
     Key=f'{automl_instance_model_path}/GradientBoostedTreeRegression_{aucroc_on_test}/metrics'
)
# We finally end up by saving the found model 
best_model.save(f"s3a://{bucket_name}/{automl_instance_model_path}/GradientBoostedTreeRegression_{aucroc_on_test}/model")
