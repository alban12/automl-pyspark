...
def generate_best_feature_subset(full_train_set_dataframe, label_column_name, task, budget):
	...
	algorithm = GeneralizedLinearRegression(maxIter=30)
...
def train_generalized_linear_regression(regParam):
	""" """
	# Unlink list
	stages = auto_fe_stages.copy()

	# Define the algorithm
	algorithm = GeneralizedLinearRegression(regParam=regParam)
...
def train_with_hyperopt(params):
	...
	model, aucroc = train_generalized_linear_regression(regParam)
	loss = aucroc
...
best_generalized_linear_regression = GeneralizedLinearRegression(regParam=best_params["regParam"])
best_generalized_linear_regression.setFeaturesCol(f"selectedFeatures")
best_generalized_linear_regression.setLabelCol(f"{label_column_name}")
best_stages = auto_fe_stages.copy()
best_stages.append(best_generalized_linear_regression)
...
s3.put_object(
     ...
     Key=f'{automl_instance_model_path}/GeneralizedLinearRegression_{aucroc_on_test}/metrics'
)
# We finally end up by saving the found model 
best_model.save(f"s3a://{bucket_name}/{automl_instance_model_path}/GeneralizedLinearRegression_{aucroc_on_test}/model")
