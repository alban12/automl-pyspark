...
# We handle the arguments received 

# Set metric
if metric == "None":
	metric = None

# Retrieve data path in S3
if train_only == "True":
...
algorithm_name = "FactorizationMachinesRegression"
...
def generate_best_feature_subset(full_train_set_dataframe, label_column_name, task, budget):
	...
	algorithm = IsotonicRegression(maxIter=30)
...
def train_fm_regression(regParam, initStd):
	""" """
	...
	algorithm = FMRegressor(regParam=regParam, initStd=initStd)
...
def train_with_hyperopt(params):
	...
	initStd = params["initStd"]

	model, aucroc = train_fm_regression(regParam, initStd)
	loss = - aucroc
...
space = {
  ...
  'initStd': hp.uniform('initStd', 0.0, 0.02*float(budget)),
}
...
best_fm_regression = FMRegressor(regParam=best_params["regParam"], initStd=best_params["initStd"])
best_fm_regression.setFeaturesCol(f"selectedFeatures")
best_fm_regression.setLabelCol(f"{label_column_name}")
best_stages = auto_fe_stages.copy()
best_stages.append(best_fm_regression)
...
s3 = boto3.client('s3')
json_object = {"Algorithm" : "FactorizationMachinesRegression",
...
s3.put_object(
     ...
     Key=f'{automl_instance_model_path}/FactorizationMachinesRegression_{aucroc_on_test}/metrics'
)
# We finally end up by saving the found model 
best_model.save(f"s3a://{bucket_name}/{automl_instance_model_path}/FactorizationMachinesRegression_{aucroc_on_test}/model")
